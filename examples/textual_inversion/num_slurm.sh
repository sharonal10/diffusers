...
for val in "${vals[@]}"; do
    python -m tu.sbatch.sbatch_sweep --time 96:00:00 \
    --proj_dir /viscam/projects/image2Blender/diffusers/examples/textual_inversion --conda_env diffusers \
    --job "08-10-infer-black-chair-${val}-LR5e-06" --command "accelerate launch textual_inversion.py   --pretrained_model_name_or_path='CompVis/stable-diffusion-v1-4'   --train_data_dir='images/black_chair_rendered' --save_as_full_pipeline  --learnable_property='object'   --placeholder_token='<chair-toy>'   --initializer_token='chair'   --resolution=512   --train_batch_size=1   --gradient_accumulation_steps=4   --max_train_steps=${val}   --learning_rate=5.0e-06   --scale_lr   --lr_scheduler='constant'   --lr_warmup_steps=0   --output_dir='results-black_chair_rendered-${val}-LR5e-06' && python inference.py --prompt 'a photo of a <chair-toy>' --model_id 'results-black_chair_rendered-${val}-LR5e-06' --output_image_name 'output_imgs/black_chair_rendered-${val}-LR5e-06.png'"  $GPU_INFO "$EXTRA_GPU_INFO"

    python -m tu.sbatch.sbatch_sweep --time 96:00:00 \
    --proj_dir /viscam/projects/image2Blender/diffusers/examples/textual_inversion --conda_env diffusers \
    --job "08-10-infer-brown-chair-${val}-LR5e-06" --command "accelerate launch textual_inversion.py   --pretrained_model_name_or_path='CompVis/stable-diffusion-v1-4'   --train_data_dir='images/brown_chair_rendered' --save_as_full_pipeline  --learnable_property='object'   --placeholder_token='<chair-toy>'   --initializer_token='chair'   --resolution=512   --train_batch_size=1   --gradient_accumulation_steps=4   --max_train_steps=${val}   --learning_rate=5.0e-06   --scale_lr   --lr_scheduler='constant'   --lr_warmup_steps=0    --output_dir='results-brown_chair_rendered-${val}-LR5e-06' && python inference.py --prompt 'a photo of a <chair-toy>' --model_id 'results-brown_chair_rendered-${val}-LR5e-06' --output_image_name 'output_imgs/brown_chair_rendered-${val}-LR5e-06.png'"  $GPU_INFO "$EXTRA_GPU_INFO"
